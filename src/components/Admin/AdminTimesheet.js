...
const AdminTimesheet = () => {
  ...
  const applyFilters = () => {
    ...
    if (filters.searchText) {
      ...
      result = result.filter(item =>
        ...
        (item.userEmail && item.userEmail.toLowerCase().includes(searchText)) ||
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `admin-timesheet-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  return (
    <div className="timesheet-container">
      ...
      <div className="timesheet-summary">
        ...
        <div className="summary-card">
          <h3>Total Hours</h3>
          <p>{filteredData.reduce((sum, item) => sum + (Number(item.actualHours) || 0), 0)}</p>
        </div>
        <div className="summary-card">
          <h3>Unique Users</h3>
          <p>{[...new Set(filteredData.map(item => item.userEmail))].length}</p>
        </div>
      </div>
